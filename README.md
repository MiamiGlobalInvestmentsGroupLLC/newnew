...
The certificate APIs now require persistent KV on Vercel (no memory fallback).
...
If KV variables are missing, API requests return an error so deployment issues are visible immediately.
